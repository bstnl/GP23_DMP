...
object TagsContext2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val verties = allUserId.flatMap(row=>{
      ...
      val adList = TagAd.makeTags(rows)
      // 商圈
      val businessList = BusinessTag.makeTags(rows)
      // 媒体标签
      val appList = TagApp.makeTags(rows,broadValue)

      // 设备标签
      val devList = TagDevice.makeTags(rows)
      // 地域标签
      val locList = TagLocation.makeTags(rows)
      // 关键字标签
      val kwList = TagKword.makeTags(rows,broadValues)
      // 获取所有的标签
